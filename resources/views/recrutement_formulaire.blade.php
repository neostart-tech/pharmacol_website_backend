...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="UTF-8">
        <title>{{ __('messages.apply_to', ['poste' => $poste->titre]) }}</title>
    ...
    <body class="bg-gray-100 p-4 sm:p-8">
        <div class="max-w-xl mx-auto bg-white shadow-md rounded-lg p-4 sm:p-6">
                ...
                <span class="font-bold tracking-wide">{{ __('messages.back_to_offers') }}</span>
            </a>
            
            <h1 class="text-xl sm:text-2xl font-bold mb-4 text-[#3C74A8] break-words">{{ __('messages.apply_title', ['poste' => $poste->titre]) }}</h1>
            ...
            <form action="https://formspree.io/f/xzzrwanv" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    <label class="block mb-1">{{ __('messages.message') }} :</label>
