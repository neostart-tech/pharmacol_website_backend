...
<html lang="fr">
    <head>
        ...
        <title>{{ __('messages.dashboard') }} - {{ __('messages.admin') }}</title>
    ...
    <body class="bg-[#f7fafc]">
        ...
        <div class="flex">
            <aside id="sidebar" class="sidebar-shadow fixed top-0 left-0 h-screen w-[18rem] bg-white text-gray-700 p-4 z-50 flex flex-col border-r border-gray-200 shadow-r transition-all duration-300">
                <div class="flex flex-col items-center mb-6">
                    <img src="{{ asset('images/Page prestations 2/logo-350100.png') }}" alt="{{ __('messages.logo_pharmacol_alt') }}" class="h-16 mb-2">
                    <span class="text-lg font-bold text-[#3C74A8] tracking-wide">{{ __('messages.admin') }}</span>
                </div>
                <nav class="flex flex-col gap-1 min-w-[200px] p-2 text-base text-gray-700 flex-1">
                    <button type="button" onclick="showSection('blog')" class="sidebar-link flex items-center w-full p-3 rounded-lg hover:bg-blue-50 hover:text-blue-900 transition">
                        <i class="fas fa-blog mr-3"></i> {{ __('messages.blog') }}
                    </button>
                    <button type="button" onclick="showSection('recrutement')" class="sidebar-link flex items-center w-full p-3 rounded-lg hover:bg-blue-50 hover:text-blue-900 transition">
                        <i class="fas fa-briefcase mr-3"></i> {{ __('messages.recruitment_monitoring') }}
                    </button>
                    <button type="button" onclick="showSection('newsletter')" class="sidebar-link flex items-center w-full p-3 rounded-lg hover:bg-blue-50 hover:text-blue-900 transition">
                        <i class="fas fa-envelope-open-text mr-3"></i> {{ __('messages.newsletter') }}
                    </button>
                    <button type="button" onclick="showSection('entreprises')" class="sidebar-link flex items-center w-full p-3 rounded-lg hover:bg-blue-50 hover:text-blue-900 transition">
                        <i class="fas fa-building mr-3"></i> {{ __('messages.companies') }}
                    </button>
                    ...
                    <button type="button" onclick="showSection('utilisateurs')" class="sidebar-link flex items-center w-full p-3 rounded-lg hover:bg-blue-50 hover:text-blue-900 transition">
                        <i class="fas fa-users mr-3"></i> {{ __('messages.users') }}
                    </button>
                    @endif
                    <button type="button" onclick="showSection('chiffres')" class="sidebar-link flex items-center w-full p-3 rounded-lg hover:bg-blue-50 hover:text-blue-900 transition">
                        <i class="fas fa-chart-bar mr-3"></i> {{ __('messages.statistics_images') }}
                    </button>
                    <div class="flex-1"></div>
                    
                    <!-- Language Switcher -->
                    <div class="border-t border-gray-200 pt-3 mb-3">
                        <p class="text-xs text-gray-500 mb-2 px-3">{{ __('messages.language') }}</p>
                        <div class="flex gap-2 px-3">
                            <a href="{{ route('lang.switch', 'fr') }}" 
                               class="flex-1 flex items-center justify-center px-3 py-2 rounded-lg border transition
                                      {{ app()->getLocale() === 'fr' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                                <span class="text-sm font-medium">FR</span>
                            </a>
                            <a href="{{ route('lang.switch', 'en') }}" 
                               class="flex-1 flex items-center justify-center px-3 py-2 rounded-lg border transition
                                      {{ app()->getLocale() === 'en' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                    ...
                    class="logout-btn flex items-center w-full p-3 rounded-lg bg-red-600 text-white hover:bg-red-700 transition mt-2 mb-2 justify-center">
            ...
            <main class="ml-[18rem] w-full p-6 flex flex-col transition-all duration-300">
                <h1 class="text-3xl font-bold text-[#3C74A8] mb-8">{{ __('messages.dashboard') }}</h1>

                <!-- Section Blog -->
                <section id="blog" class="section-content hidden">
                    <div class="card bg-white p-6 mb-8">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-2xl font-semibold text-[#437305]">{{ __('messages.blog_articles') }}</h2>
                            <a href="{{ route('admin.blog.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow">
                                <i class="fas fa-plus mr-2"></i> {{ __('messages.add_article') }}
                            </a>
                        </div>
                            ...
                            <table class="min-w-full table-auto">
                                <thead>
                                    <tr class="bg-gray-800 text-gray-300">
                                        ...
                                        <th class="px-4 py-2">{{ __('messages.state') }}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b table-row-hover">
                                        <td class="px-4 py-2">
                                            @if($blog->image)
                                                <img src="{{ asset($blog->image) }}" alt="{{ __('messages.article_image_alt') }}" class="w-16 h-16 object-cover rounded">
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-2 space-x-2 flex gap-2">
                                            <a href="{{ route('admin.blog.edit', $blog->id) }}" class="text-blue-600 hover:underline">{{ __('messages.modify') }}</a>
                                            <form method="POST" action="{{ route('admin.blog.destroy', $blog->id) }}" onsubmit="return confirm('{{ __('messages.delete_article_confirm') }}')">
                                                @csrf
                                                @method('DELETE')
                ...
                <section id="recrutement" class="section-content hidden">
                    <div class="card bg-white p-6 mb-8">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-2xl font-semibold text-[#437305]">{{ __('messages.positions_available') }}</h2>
                            <a href="{{ route('admin.poste.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow">
                                <i class="fas fa-plus mr-2"></i> {{ __('messages.add_position') }}
                            </a>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full table-auto">
                                <thead>
                                    <tr class="bg-gray-800 text-gray-300">
                                        ...
                                        <th class="px-4 py-2">{{ __('messages.location') }}</th>
                                        <th class="px-4 py-2">{{ __('messages.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b table-row-hover">
                                        ...
                                        <td class="px-4 py-2 space-x-2 flex gap-2">
                                            <a href="{{ route('admin.poste.edit', $poste->id) }}" class="text-blue-600 hover:underline">{{ __('messages.modify') }}</a>
                                            <form method="POST" action="{{ route('admin.poste.destroy', $poste->id) }}" onsubmit="return confirm('{{ __('messages.delete_position_confirm') }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:underline">{{ __('messages.delete') }}</button>
                                            </form>
                ...
                <section id="newsletter" class="section-content hidden">
                    <div class="card bg-white p-6 mb-8">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-2xl font-semibold text-[#437305]">{{ __('messages.newsletter') }}</h2>
                            <a href="{{ route('admin.newsletter.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow">
                                <i class="fas fa-plus mr-2"></i> {{ __('messages.add') }}
                            </a>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full table-auto">
                                <thead class="bg-gray-800 text-gray-300">
                                    <tr>
                                        <th class="px-6 py-3 text-left">{{ __('messages.email') }}</th>
                                        <th class="px-6 py-3 text-left">{{ __('messages.first_name') }}</th>
                                        <th class="px-6 py-3 text-left">{{ __('messages.last_name') }}</th>
                                    ...
                                    @foreach($newsletters as $newsletter)
                                        <tr class="bg-white border-b border-gray-300 table-row-hover">
                                            ...
                                            <td class="px-6 py-3 text-center flex gap-2 justify-center">
                                                <a href="{{ route('admin.newsletter.edit', $newsletter->mail) }}" class="text-blue-600 hover:underline">{{ __('messages.modify') }}</a>
                                                <form method="POST" action="{{ route('admin.newsletter.destroy', $newsletter->mail) }}" onsubmit="return confirm('{{ __('messages.delete_newsletter_confirm') }}')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:underline">{{ __('messages.delete') }}</button>
                                                </form>
                ...
                <section id="entreprises" class="section-content hidden">
                    <div class="card bg-white p-6 mb-8">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-2xl font-semibold text-[#437305]">{{ __('messages.companies') }}</h2>
                            <a href="{{ route('admin.entreprise.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded shadow">
                                <i class="fas fa-plus mr-2"></i> {{ __('messages.add_company') }}
                            </a>
                        </div>
                                    ...
                                    <table class="min-w-full table-auto mb-4">
                                        <thead>
                                            <tr class="bg-gray-800 text-gray-300">
                                                ...
                                                <th class="px-4 py-2">{{ __('messages.city') }}</th>
                                                <th class="px-4 py-2">{{ __('messages.coordinates') }}</th>
                                                <th class="px-4 py-2">{{ __('messages.actions') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr class="bg-white border-b table-row-hover">
                                                ...
                                                <td class="px-4 py-2 flex gap-2">
                                                    <a href="{{ route('admin.entreprise.edit', $entreprise->id) }}" class="text-blue-600 hover:underline">{{ __('messages.modify') }}</a>
                                                    <form method="POST" action="{{ route('admin.entreprise.destroy', $entreprise->id) }}" onsubmit="return confirm('{{ __('messages.delete_company_confirm') }}')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:underline">{{ __('messages.delete') }}</button>
                                                    </form>
                        ...
                        <div class="mt-8">
                            <h3 class="text-xl font-bold mb-4 text-center">{{ __('messages.company_map') }}</h3>
                ...
                <section id="utilisateurs" class="section-content hidden">
                    <div class="card bg-white p-6 mb-8">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-2xl font-semibold text-[#437305]">{{ __('messages.users') }}</h2>
                            <form method="POST" action="{{ route('admin.utilisateur.store') }}" class="flex gap-2 items-end">
                                @csrf
                                <div>
                                    <label class="block text-xs">{{ __('messages.email') }}</label>
                                    <input type="email" name="mail" class="border rounded p-1" required>
                                </div>
                                <div>
                                    <label class="block text-xs">{{ __('messages.password') }}</label>
                                    <input type="password" name="mot_de_passe" class="border rounded p-1" required>
                                </div>
                                <div>
                                    <label class="block text-xs">{{ __('messages.role') }}</label>
                                    <select name="role" class="border rounded p-1" required>
                                        <option value="user">user</option>
                                        <option value="admin">admin</option>
                                    </select>
                                </div>
                                <button type="submit" class="bg-blue-600 text-white px-3 py-1 rounded">{{ __('messages.create') }}</button>
                            </form>
                        </div>
                        @if($errors->any())
                                ...
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b table-row-hover">
                                        ...
                                        <td class="px-4 py-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.utilisateur.destroy', $user->mail) }}" onsubmit="return confirm('{{ __('messages.delete_user_confirm') }}')" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:underline">{{ __('messages.delete') }}</button>
                                            </form>
                                            @else
                                            <span class="text-gray-400 text-xs">{{ __('messages.you') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                    @endforeach
                ...
                <section id="chiffres" class="section-content hidden">
                    <div class="card bg-white p-6 mb-8">
                        <h2 class="text-2xl font-semibold text-[#437305] mb-6">{{ __('messages.statistics_and_images') }}</h2>
                        <form id="chiffres-form" method="POST" enctype="multipart/form-data" action="{{ route('admin.chiffres.update') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                                @foreach(['Togo','Bénin','Niger'] as $pays)
                                    ...
                                    <div class="border rounded-lg p-4 shadow-sm">
                                        ...
                                        <label class="block mb-2 text-sm">{{ __('messages.team_image') }} (.png, .jpg, .jpeg)</label>
                                        <div class="mb-2">
                                            @if($imgPath)
                                ...
                                <div class="border rounded-lg p-4 shadow-sm md:col-span-3">
                                    <h3 class="text-lg font-bold mb-4 text-[#31689B]">{{ __('messages.general_information') }}</h3>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            <label class="block mb-2 text-sm">{{ __('messages.years_of_experience') }}</label>
                                        ...
                                        <div>
                                            <label class="block mb-2 text-sm">{{ __('messages.countries_covered') }}</label>
                                    ...
                                    <div class="md:col-span-3 mt-4">
                                        <label class="block mb-2 text-sm">{{ __('messages.expertise_description') }}</label>
                                    ...
                                    <div class="md:col-span-3 mt-4">
                                        <h4 class="text-lg font-semibold mb-4 text-[#31689B]">{{ __('messages.social_networks') }}</h4>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <label class="block mb-2 text-sm">{{ __('messages.facebook_url') }}</label>
                                            ...
                                            <div>
                                                <label class="block mb-2 text-sm">{{ __('messages.instagram_url') }}</label>
                                            ...
                                            <div>
                                                <label class="block mb-2 text-sm">{{ __('messages.linkedin_url') }}</label>
                                            ...
                                            <div>
                                                <label class="block mb-2 text-sm">{{ __('messages.youtube_url') }}</label>
                                    ...
                                    <div class="md:col-span-3 mt-4">
                                        <h4 class="text-lg font-semibold mb-4 text-[#31689B]">{{ __('messages.contact') }}</h4>
                                        <div>
                                            <label class="block mb-2 text-sm">{{ __('messages.contact_email') }}</label>
                            ...
                            <button type="submit" class="mt-6 bg-[#437305] hover:bg-[#31689B] text-white px-6 py-2 rounded shadow">{{ __('messages.save') }}</button>
                        </form>
                    </div>
                    ...
                    <div class="card bg-white p-6 mt-8">
                        <div class="flex justify-between items-center mb-6">
                            <h2 class="text-2xl font-semibold text-[#437305]">{{ __('messages.partner_management') }}</h2>
                            <button id="add-partenaire-btn" class="bg-[#437305] hover:bg-[#31689B] text-white px-4 py-2 rounded shadow">
                                <i class="fas fa-plus mr-2"></i>{{ __('messages.add_partner') }}
                            </button>
                        </div>
                ...
                <div id="partenaire-modal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden">
                    <div class="flex items-center justify-center min-h-screen p-4">
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <h3 id="modal-title" class="text-xl font-semibold text-[#437305]">{{ __('messages.add_partner') }}</h3>
                        ...
                        <form id="partenaire-form" enctype="multipart/form-data">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium mb-2">{{ __('messages.partner_name') }}</label>
                                    <input type="text" id="partenaire-nom" name="nom" class="w-full border rounded p-2" required>
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium mb-2">{{ __('messages.website_link') }}</label>
                                    <input type="url" id="partenaire-lien" name="lien" class="w-full border rounded p-2" placeholder="https://" required>
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium mb-2">{{ __('messages.logo_image') }}</label>
                                    ...
                                    <div id="current-image" class="mt-2 hidden">
                                        <img id="preview-image" src="" alt="{{ __('messages.preview') }}" class="max-h-20 object-contain">
                                    </div>
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium mb-2">{{ __('messages.display_order') }}</label>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <label for="partenaire-actif" class="text-sm font-medium">{{ __('messages.active') }}</label>
                                </div>
                            </div>
                            
                            <div class="flex justify-end space-x-2 mt-6">
                                <button type="button" id="cancel-btn" class="px-4 py-2 border border-gray-300 rounded text-gray-700 hover:bg-gray-50">{{ __('messages.cancel') }}</button>
                                <button type="submit" class="px-4 py-2 bg-[#437305] text-white rounded hover:bg-[#31689B]">{{ __('messages.register') }}</button>
                            </div>
                        </form>
                    </div>
                    </div>
        ...
        async function loadPartenaires() {
            ...
            try {
                ...
                if (partenaires.length === 0) {
                    listContainer.innerHTML = '<p class="text-gray-500 text-center py-4">{{ __('messages.no_partners_configured') }}</p>';
                    return;
                }
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <span class="text-xs ${partenaire.actif ? 'text-green-600' : 'text-red-600'}">
                                    ${partenaire.actif ? '{{ __('messages.active') }}' : '{{ __('messages.inactive') }}'}
                                </span>
                            </div>
                        </div>
                        <div class="flex space-x-2">
                            <button onclick="editPartenaire(${partenaire.id})" class="text-blue-600 hover:underline">{{ __('messages.modify') }}</button>
                            <button onclick="deletePartenaire(${partenaire.id})" class="text-red-600 hover:underline">{{ __('messages.delete') }}</button>
                        </div>
                    `;
                ...
                const listContainer = document.getElementById('partenaires-list');
                if (listContainer) {
                    listContainer.innerHTML = '<p class="text-red-500 text-center py-4">{{ __('messages.loading_error') }}: ' + error.message + '</p>';
                }
            }
        }

        // Ouvrir le modal
        function openPartenaireModal(title = '{{ __('messages.add_partner') }}') {
        ...
        async function editPartenaire(id) {
            try {
                ...
                if (partenaire) {
                    ...
                    openPartenaireModal('{{ __('messages.modify_partner') }}');
                }
            } catch (error) {
        ...
        async function deletePartenaire(id) {
            if (!confirm('{{ __('messages.delete_partner_confirm') }}')) return;
